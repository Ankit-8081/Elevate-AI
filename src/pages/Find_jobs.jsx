...
import { 
  Search, MapPin, Briefcase, Globe, Filter, 
  Sparkles, Bookmark, ExternalLink, ChevronRight, 
  ...
  const handleSearch = async (e) => {
    ...
    setTimeout(() => {
      const mockJobs = [
        ...
        { id: 2, title: "Senior AI Researcher", company: "Anthropic", location: "Remote", salary: "$180k - $240k", source: "Web", match_score: 74, description: "Focus on safety-first alignment...", skills: ["RLHF", "NLP"] },
        { id: 3, title: "Data Scientist", company: "Zomato", location: "Gurgaon", salary: "₹15–25 LPA", source: "Naukri", match_score: 58, description: "Optimize delivery routes using graph networks...", skills: ["SQL", "Pandas"] },
      ];
      setJobs(mockJobs);
      setLoading(false);
    }, 1500);
              ...
              <AnimatePresence>
                {loading ? (
                  <div className="col-span-full py-20 text-center space-y-4">
                    ...
                    <p className="text-slate-400 animate-pulse">Searching across LinkedIn, Naukri, and Web jobs...</p>
                  </div>
                ) : jobs.length > 0 ? (
                  jobs.map((job) => (
...
const JobCard = ({ job, onOpen }) => {
  ...
  return (
    <div
  className="group relative p-6 rounded-2xl bg-white/5 border border-white/10 hover:border-indigo-500/50 hover:bg-white/[0.07] transition-all overflow-hidden">
    
      <div className="flex justify-between items-start mb-4">
        <div className="w-12 h-12 rounded-xl bg-indigo-500/20 flex items-center justify-center border border-indigo-500/30">
          <Briefcase className="text-indigo-400" size={24} />
        </div>
        <div className={`px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider ${getScoreColor(job.match_score)}`}>
          {job.match_score}% Match
        </div>
      </div>

      <h3 className="text-xl font-semibold text-white group-hover:text-indigo-300 transition-colors leading-snug">
        {job.title}
      </h3>
      ...
      <div className="mt-4 flex flex-wrap gap-3 text-sm text-slate-500">
        ...
        <div className="flex items-center gap-1"><Globe size={14} /> {job.source}</div>
      </div>
        ...
        <div className="p-8 space-y-6">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-indigo-400 font-medium mt-1">{job.company} • {job.location}</p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/10">
              <p className="text-xs text-emerald-500 font-bold uppercase mb-2">Strengths</p>
              <ul className="text-sm space-y-2 text-slate-300">
                <li className="flex items-center gap-2"><CheckCircle2 size={14} /> Matches your {job.skills[0]} experience</li>
                <li className="flex items-center gap-2"><CheckCircle2 size={14} /> Top 10% Match for this role</li>
              </ul>
            </div>
            <div className="p-4 rounded-2xl bg-amber-500/5 border border-amber-500/10">
              <p className="text-xs text-amber-500 font-bold uppercase mb-2">Missing Skills</p>
              <ul className="text-sm space-y-2 text-slate-300">
                <li className="flex items-center gap-2 text-slate-400 opacity-80"><AlertCircle size={14} /> Cloud Architecture</li>
              </ul>
            </div>
          </div>
          ...
          <div className="flex gap-4 pt-4 border-t border-white/5">
            <button className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-indigo-600/20">
              Apply on {job.source}
            </button>
